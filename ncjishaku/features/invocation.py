...
"""
ncjishaku.features.invocation
~~~~~~~~~~~~~~~~~~~~~~~~~~~~

The ncjishaku command invocation related commands.
...
from ncjishaku.types import ContextA, ContextT
...
class InvocationFeature(Feature):
    ...
    async def jsk_repeat(self, ctx: ContextT, times: int, *, command_string: str):
        ...
        with self.submit(ctx):  # pyright: ignore[reportArgumentType] # allow repeats to be cancelled
    ...
    async def jsk_debug(self, ctx: ContextT, *, command_string: str):
        ...
        async with ReplResponseReactor(ctx.message):
            with self.submit(ctx):  # pyright: ignore[reportArgumentType]
                await alt_ctx.command.invoke(alt_ctx)
